...
function injectEventPlugins(eventPlugins) {
  for (var i = 0; i < eventPlugins.length; i++) {
    var pluginModule = eventPlugins[i];
...
var USE_EVENT_SYSTEM = 1 << 2;
var IS_TARGET_PHASE_ONLY = 1 << 3;
var IS_REPLAYED = 1 << 6;
...
{
  otherDiscreteEvents.push(TOP_BEFORE_BLUR, TOP_AFTER_BLUR);
} // prettier-ignore
...
function getEventPriorityForListenerSystem(type) {
  var priority = eventPriorities.get(type);
  ...
  {
    warn('The event "type" provided to useEvent() does not have a known priority type.' + ' It is recommended to provide a "priority" option to specify a priority.');
  }

  return ContinuousEvent;
}

var capturePhaseEvents = new Set([TOP_FOCUS, TOP_BLUR, TOP_SCROLL, TOP_LOAD, TOP_ABORT, TOP_CANCEL, TOP_CLOSE, TOP_INVALID, TOP_RESET, TOP_SUBMIT, TOP_ABORT, TOP_CAN_PLAY, TOP_CAN_PLAY_THROUGH, TOP_DURATION_CHANGE, TOP_EMPTIED, TOP_ENCRYPTED, TOP_ENDED, TOP_ERROR, TOP_LOADED_DATA, TOP_LOADED_METADATA, TOP_LOAD_START, TOP_PAUSE, TOP_PLAY, TOP_PLAYING, TOP_PROGRESS, TOP_RATE_CHANGE, TOP_SEEKED, TOP_SEEKING, TOP_STALLED, TOP_SUSPEND, TOP_TIME_UPDATE, TOP_VOLUME_CHANGE, TOP_WAITING]);
...
var isArray = Array.isArray; // TODO: we should remove the FlowFixMes and the casting to figure out how to make
...
var eventTargetEventListenerStore = new PossiblyWeakMap$1(); // $FlowFixMe: Flow cannot handle polymorphic WeakMaps

var reactScopeListenerStore = new PossiblyWeakMap$1();
...
function executeDispatchesInOrder(event) {
  ...
  var dispatchCurrentTargets = event._dispatchCurrentTargets;
  var previousInstance;

  if (dispatchListeners !== null && dispatchInstances !== null && dispatchCurrentTargets !== null) {
    for (var i = 0; i < dispatchListeners.length; i++) {
      var instance = dispatchInstances[i];
      var listener = dispatchListeners[i];
      var currentTarget = dispatchCurrentTargets[i]; // We check if the instance was the same as the last one,
...
function dispatchEventsInBatch(events) {
  for (var i = 0; i < events.length; i++) {
    var syntheticEvent = events[i];
...
function dispatchEventsForPlugins(topLevelType, eventSystemFlags, nativeEvent, targetInst, targetContainer) {
  ...
  for (var i = 0; i < plugins.length; i++) {
    ...
    if (possiblePlugin !== undefined) {
      var extractedEvents = possiblePlugin.extractEvents(topLevelType, targetInst, nativeEvent, nativeEventTarget, eventSystemFlags, targetContainer);
...
function listenToTopLevelEvent(topLevelType, targetContainer, listenerMap, eventSystemFlags, passive, priority, capture) {
  ...
  var listenerMapKey = capture === undefined ? topLevelType : typeStr + "_" + (capture ? 'capture' : 'bubble');
  ...
  if (listenerEntry === undefined || shouldUpgrade) {
    var isCapturePhase = capture === undefined ? capturePhaseEvents.has(topLevelType) : capture; // If we should upgrade, then we need to remove the existing trapped
    // event listener for the target container.

    if (shouldUpgrade) {
      removeTrappedEventListener(targetContainer, topLevelType, isCapturePhase, listenerEntry.listener);
    }

    var listener = addTrappedEventListener(targetContainer, topLevelType, eventSystemFlags, isCapturePhase, false, passive, priority);
...
function listenToEvent(registrationName, rootContainerElement) {
  ...
  for (var i = 0; i < dependencies.length; i++) {
    var dependency = dependencies[i];
    listenToTopLevelEvent(dependency, rootContainerElement, listenerMap, PLUGIN_EVENT_SYSTEM);
  }
}
...
function isManagedDOMElement(target) {
  return getClosestInstanceFromNode(target) !== null;
}
...
function dispatchEventForPluginEventSystem(topLevelType, eventSystemFlags, nativeEvent, targetInst, targetContainer) {
  ...
  batchedEventUpdates(function () {
    return dispatchEventsForPlugins(topLevelType, eventSystemFlags, nativeEvent, ancestorInst, targetContainer);
  });
}

function getNearestRootOrPortalContainer(instance) {
  var node = getClosestInstanceFromNode(instance);

  while (node !== null) {
    var tag = node.tag; // Once we encounter a host container or root container
...
function addEventTypeToDispatchConfig(type) {
  var dispatchConfig = topLevelEventsToDispatchConfig.get(type); // If we don't have a dispatchConfig, then we're dealing with
  ...
  if (dispatchConfig === undefined) {
    topLevelEventsToDispatchConfig.set(type, emptyDispatchConfigForCustomEvents);
  }
}

function attachListenerToManagedDOMElement(listener) {
  var event = listener.event,
      target = listener.target;
  var passive = event.passive,
      priority = event.priority,
      type = event.type;
  var managedTargetElement = target;
  var containerEventTarget = getNearestRootOrPortalContainer(managedTargetElement);
  var listenerMap = getListenerMapForElement(containerEventTarget); // Add the event listener to the target container (falling back to
  // the target if we didn't find one).

  listenToTopLevelEvent(type, containerEventTarget, listenerMap, PLUGIN_EVENT_SYSTEM | USE_EVENT_SYSTEM, passive, priority); // Get the internal listeners Set from the target instance.

  var listeners = getListenersFromTarget(managedTargetElement); // If we don't have any listeners, then we need to init them.
  ...
  listeners.add(listener); // Finally, add the event to our known event types list.

  addEventTypeToDispatchConfig(type);
}
function detachListenerFromManagedDOMElement(listener) {
  var target = listener.target;
  var managedTargetElement = target; // Get the internal listeners Set from the target instance.

  var listeners = getListenersFromTarget(managedTargetElement);
...
function attachTargetEventListener(listener) {
  var event = listener.event,
      target = listener.target;
  ...
  var listenerMap = getListenerMapForElement(eventTarget); // Add the event listener to the TargetEvent object.

  listenToTopLevelEvent(type, eventTarget, listenerMap, PLUGIN_EVENT_SYSTEM | USE_EVENT_SYSTEM | IS_TARGET_PHASE_ONLY, passive, priority, capture);
  var eventTypeMap = eventTargetEventListenerStore.get(eventTarget);
  ...
  var listeners = eventTypeMap.get(type);
...
function detachTargetEventListener(listener) {
  var event = listener.event,
      target = listener.target;
  var capture = event.capture,
      type = event.type;
  var validEventTarget = target;
  var eventTypeMap = eventTargetEventListenerStore.get(validEventTarget);
...
function attachListenerToReactScope(listener) {
  var event = listener.event,
      target = listener.target;
  var capture = event.capture,
      type = event.type;
  var reactScope = target;
  var eventTypeMap = reactScopeListenerStore.get(reactScope);
...
function detachListenerFromReactScope(listener) {
  var event = listener.event,
      target = listener.target;
  var capture = event.capture,
      type = event.type;
...
function trapReplayableEventForContainer(topLevelType, container, listenerMap) {
  listenToTopLevelEvent(topLevelType, container, listenerMap, PLUGIN_EVENT_SYSTEM);
}
...
function eagerlyTrapReplayableEvents(container, document) {
  ...
  discreteReplayableEvents.forEach(function (topLevelType) {
    {
      trapReplayableEventForContainer(topLevelType, container, listenerMapForContainer);
    }
  }); // Continuous
...
function addTrappedEventListener(targetContainer, topLevelType, eventSystemFlags, capture, isDeferredListenerForLegacyFBSupport, passive, priority) {
  ...
  if (capture) {
    if ( passive !== undefined) {
      ...
      unsubscribeListener = addEventCaptureListenerWithPassiveFlag(targetContainer, rawEventName, listener, passive);
    } else {
    ...
    if ( passive !== undefined) {
      // This is only used with passive is either true or false.
      unsubscribeListener = addEventBubbleListenerWithPassiveFlag(targetContainer, rawEventName, listener, passive);
    } else {
...
function removeTrappedEventListener(targetContainer, topLevelType, capture, listener) {
  var rawEventName = getRawEventName(topLevelType);
...
function dispatchEvent(topLevelType, eventSystemFlags, targetContainer, nativeEvent) {
  ...
  {
    {
      dispatchEventForPluginEventSystem(topLevelType, eventSystemFlags, nativeEvent, null, targetContainer);
    }
  }
...
function attemptToDispatchEvent(topLevelType, eventSystemFlags, targetContainer, nativeEvent) {
  ...
  {
    {
      dispatchEventForPluginEventSystem(topLevelType, eventSystemFlags, nativeEvent, targetInst, targetContainer);
    }
...
function ensureListeningTo(rootContainerInstance, registrationName) {
  {
    ...
    var rootContainerElement = rootContainerInstance.nodeType === COMMENT_NODE ? rootContainerInstance.parentNode : rootContainerInstance; // Containers can only ever be element nodes. We do not
...
function beforeRemoveInstance(instance) {
...

  {
    ...
    var listenersSet = getListenersFromTarget(instance);
...
function registerEvent(event, rootContainerInstance) {
  var passive = event.passive,
      priority = event.priority,
      type = event.type;
  var listenerMap = getListenerMapForElement(rootContainerInstance); // Add the event listener to the target container (falling back to
  // the target if we didn't find one).

  listenToTopLevelEvent(type, rootContainerInstance, listenerMap, PLUGIN_EVENT_SYSTEM | USE_EVENT_SYSTEM, passive, priority);
}
function mountEventListener(listener) {
  {
    var target = listener.target;
...
function unmountEventListener(listener) {
  {
    var target = listener.target;
...
function validateEventListenerTarget(target, listener) {
  {
    if (target != null && (isManagedDOMElement(target) || isValidEventTarget(target) || isReactScope(target))) {
      if (listener == null || typeof listener === 'function') {
        return true;
      }

      {
        warn('Event listener method setListener() from useEvent() hook requires the second argument' + ' to be either a valid function callback or null/undefined.');
      }
    }

    {
      warn('Event listener method setListener() from useEvent() hook requires the first argument to be ' + 'a valid DOM EventTarget. If using a ref, ensure the current value is not null.');
    }
  }
...
var internalEventListenersKey = '__reactListeners$' + randomKey;
...
function getListenersFromTarget(target) {
  return target[internalEventListenersKey] || null;
} // This is used for useEvent listeners

function initListenersSet(target, value) {
  target[internalEventListenersKey] = value;
}
...
function accumulateTwoPhaseListeners(event, accumulateUseEventListeners) {
  ...
  var instance = event._targetInst;
  ...
  while (instance !== null) {
    ...
    if (tag === HostComponent && stateNode !== null) {
      ...
      lastHostComponent = currentTarget; // For useEvent listenrs

      if ( accumulateUseEventListeners) {
        // useEvent event listeners
        var targetType = event.type;
        var listeners = getListenersFromTarget(currentTarget);

        if (listeners !== null) {
          var listenersArr = Array.from(listeners);

          for (var i = 0; i < listenersArr.length; i++) {
            var listener = listenersArr[i];
            var callback = listener.callback,
                _listener$event = listener.event,
                capture = _listener$event.capture,
                type = _listener$event.type;
    ...
    if ( accumulateUseEventListeners && tag === ScopeComponent && lastHostComponent !== null) {
      var reactScope = stateNode.methods;
      var eventTypeMap = reactScopeListenerStore.get(reactScope);

      if (eventTypeMap !== undefined) {
        var _type = event.type;

        var _listeners = eventTypeMap.get(_type);

        if (_listeners !== undefined) {
          var captureListeners = Array.from(_listeners.captured);
          var bubbleListeners = Array.from(_listeners.bubbled);
          var lastCurrentTarget = lastHostComponent;

          for (var _i = 0; _i < captureListeners.length; _i++) {
            var _listener = captureListeners[_i];
            var _callback = _listener.callback;
            dispatchListeners.unshift(_callback);
            dispatchInstances.unshift(instance);
            dispatchCurrentTargets.unshift(lastCurrentTarget);
          }

          for (var _i2 = 0; _i2 < bubbleListeners.length; _i2++) {
            var _listener2 = bubbleListeners[_i2];
            var _callback2 = _listener2.callback;
            dispatchListeners.push(_callback2);
            dispatchInstances.push(instance);
...
function accumulateEnterLeaveListenersForEvent(event, target, common, capture) {
...
function accumulateEventTargetListeners(event, currentTarget) {
  var dispatchListeners = [];
  var dispatchInstances = [];
  var dispatchCurrentTargets = [];
  var eventTypeMap = eventTargetEventListenerStore.get(currentTarget);
  ...
  if (eventTypeMap !== undefined) {
    ...
    if (listeners !== undefined) {
      var isCapturePhase = event.eventPhase === 1;

      if (isCapturePhase) {
        var captureListeners = Array.from(listeners.captured);

        for (var i = captureListeners.length - 1; i >= 0; i--) {
          var listener = captureListeners[i];
          var callback = listener.callback;
          dispatchListeners.push(callback); // EventTarget listeners do not have instances, as there
        ...
        var bubbleListeners = Array.from(listeners.bubbled);

        for (var _i = 0; _i < bubbleListeners.length; _i++) {
          var _listener = bubbleListeners[_i];
          var _callback = _listener.callback;
          dispatchListeners.push(_callback); // EventTarget listeners do not have instances, as there
          // is no backing Fiber instance for them (window, document etc).
...
var SimpleEventPlugin = {
  ...
  extractEvents: function (topLevelType, targetInst, nativeEvent, nativeEventTarget, eventSystemFlags, targetContainer) {
    ...
    var event = EventConstructor.getPooled(dispatchConfig, targetInst, nativeEvent, nativeEventTarget); // For TargetEvent only accumulation, we do not traverse through
    ...
    if ( eventSystemFlags !== undefined && eventSystemFlags & IS_TARGET_PHASE_ONLY && targetContainer != null) {
...
{
  injectEventPlugins([SimpleEventPlugin, EnterLeaveEventPlugin, ChangeEventPlugin, SelectEventPlugin, BeforeInputEventPlugin]);
}
...
var noOpMount = function () {};

function validateNotInFunctionRender() {
  if (currentlyRenderingFiber$1 === null) {
    return true;
  }

  {
    warn('Event listener methods from useEvent() cannot be called during render.' + ' These methods should be called in an effect or event callback outside the render.');
  }

  return false;
}

function createReactListener(event, callback, target, destroy) {
  return {
    callback: callback,
    destroy: destroy,
    event: event,
...
function mountEventListener$1(event) {
  {
    ...
    var rootContainerInstance = getRootHostContainer(); // Register the event to the current root to ensure event
    ...
    var clear = function () {
      if (validateNotInFunctionRender()) {
        var listeners = Array.from(listenerMap.values());

        for (var i = 0; i < listeners.length; i++) {
          unmountEventListener(listeners[i]);
        }
    ...
    var reactListenerMap = {
      clear: clear,
      setListener: function (target, callback) {
        if (validateNotInFunctionRender() && validateEventListenerTarget(target, callback)) {
          var listener = listenerMap.get(target);
    ...
    currentlyRenderingFiber$1.effectTag |= Update;
    pushEffect(NoEffect$1, noOpMount, clear, null);
...
function updateEventListener(event) {
  {
    var hook = updateWorkInProgressHook();
    var _hook$memoizedState = hook.memoizedState,
        reactListenerMap = _hook$memoizedState[0],
        memoizedEvent = _hook$memoizedState[1],
        clear = _hook$memoizedState[2];

    {
      if (memoizedEvent.type !== event.type) {
        warn('The event type argument passed to the useEvent() hook was different between renders.' + ' The event type is static and should never change between renders.');
      }

      if (memoizedEvent.capture !== event.capture) {
        warn('The "capture" option passed to the useEvent() hook was different between renders.' + ' The "capture" option is static and should never change between renders.');
      }

      if (memoizedEvent.priority !== event.priority) {
        warn('The "priority" option passed to the useEvent() hook was different between renders.' + ' The "priority" option is static and should never change between renders.');
      }

      if (memoizedEvent.passive !== event.passive) {
        warn('The "passive" option passed to the useEvent() hook was different between renders.' + ' The "passive" option is static and should never change between renders.');
      }
    } // In order to clear up upon the hook unmounting,
    // we ensure we set the effecrt tag so that we visit
    // this effect in the commit phase, so we can handle
...

{
  ...
  HooksDispatcherOnMountInDEV = {
    ...
    useEvent: function (event) {
      ...
      mountHookTypesDev();
      return mountEventListener$1(event);
    },
...
function resolveDispatcher() {
  var dispatcher = ReactCurrentDispatcher$2.current;
...
function useEvent(type, options) {
  var dispatcher = resolveDispatcher();
  var capture = false;
  var passive = undefined; // Undefined means to use the browser default

  var priority;

  if (options != null) {
    var optionsCapture = options.capture;
    var optionsPassive = options.passive;
    var optionsPriority = options.priority;
...
var Internals = {
  ...
  Events: [getInstanceFromNode$1, getNodeFromInstance, getFiberCurrentPropsFromNode, injectEventPluginsByName, eventNameDispatchConfigs, enqueueStateRestore, restoreStateIfNeeded, dispatchEvent, flushPassiveEffects, IsThisRendererActing]
};
...
exports.unstable_useEvent = useEvent;
