...
var enableFlareAPI = true; // Experimental Host Component support.

var enableFundamentalAPI = true; // New API for JSX transforms to target - https://github.com/reactjs/rfcs/pull/107
...
var enableUserBlockingEvents = true; // Add a callback property to suspense to notify which promises are currently
...
function mountEventResponder$1(responder, responderProps, instance, fiber, respondersMap) {
  ...
  mountResponderInstance(responder, responderInstance, responderProps, responderState, instance);
  respondersMap.set(responder, responderInstance);
}

function updateEventListener(listener, fiber, visistedResponders, respondersMap, instance) {
  var responder;
  var props;
  ...
  if (responderInstance === undefined) {
    ...
    mountEventResponder$1(responder, listenerProps, instance, fiber, respondersMap);
  } else {
  ...
  if (listeners != null) {
    ...
    if (isArray$2(listeners)) {
      for (var i = 0, length = listeners.length; i < length; i++) {
        ...
        updateEventListener(listener, fiber, visistedResponders, respondersMap, instance);
      }
    } else {
      updateEventListener(listeners, fiber, visistedResponders, respondersMap, instance);
    }
  }
