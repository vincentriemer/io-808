...
class TopRightSection extends React.Component {
  static propTypes = {
    ...
    seperatorWidth: React.PropTypes.number.isRequired
  };
  ...
  static generateInstrumentColumns(seperatorWidth, instrumentColumnWidth, instrumentColumnHeight) {
    const instrumentSeperatorHeight = instrumentColumnHeight - 10;

    const seperatorStyle = {
      width: seperatorWidth, height: instrumentSeperatorHeight,
      backgroundColor: grey
    };

    return TopRightSection.instrumentConfig.reduce((components, config, index) => {
      if (index !== 0)
      ...
      components.push(
        <ConnectedInstrumentColumn
          key={`column-${index}`}
          config={config}
          width={instrumentColumnWidth} height={instrumentColumnHeight} />
      );
      return components;
    }, []);
  };
  ...
  static connectedMasterVolumeKnob() {
    ...
    return connect(mapStateToProps, mapDispatchToProps)(MasterVolumeKnob);
  };
  ...
  render() {
    ...
    const styles = {
      ...
      instrumentsWrapper: {
        width: width, height: instrumentsHeight,
        display: 'flex',
        flexDirection: 'row',
        alignItems: 'center'
      },
      ...
      titleWrapper: {
        width: width, height: titleSectionHeight,
        display: 'flex',
        flexDirection: 'row',
        alignItems: 'center',
    ...
    return (
      <div style={styles.wrapper}>
        <div style={styles.instrumentsWrapper}>
             {TopRightSection.generateInstrumentColumns(seperatorWidth, instrumentColumnWidth, instrumentsHeight)}
        </div>
